...
# --- Utility functions for loading data ---
  ...
  #' It supports loading from CSV, RDS, and ZIP files containing these formats. 
  ...
  check_and_load <- function(file_path, name = NULL, load_function = NULL, envir = globalenv(), force_reload = FALSE, ...) {
    ...
    if (extension == "zip") {
      ...
      valid_files <- extracted_files[grepl("\\.(csv|rds)$", extracted_files)]
      
      if (length(valid_files) == 0) {
        stop("No CSV or RDS files found in the ZIP archive.")
      }
  ...
  load_models <- function(session, model_names, model_paths, file_upload = FALSE) {
    if(!file_upload) {
      ...
      model_list <- lapply(seq_along(model_names), function(i) {
        obj <- validate_and_read_rds(session = session, file_path = model_paths[[i]])
        return(obj)
      })
    }
...
# --- Functions for loading specific objects or files ---
  ...
  load_response_fermentation <- function() {
    ...
    print("load_response_fermentation") # debug
  ...
  load_response_methanogenesis <- function() {
    ...
    print("load_response_methanogenesis") # debug
