...
helpServer <- function(input, output, session, x, selected_tab) {
  ...
  output$main_content <- shiny::renderUI({
    switch(input$subtabs,
           ...
           "Predict traits from taxonomy" = div(
             ...
             p("This tool predicts traits for organisms given their taxonomy. After the user selects organisms for prediction, the tool finds organisms with matching taxonomy in the internal database. If the matching organisms share a trait, the trait is predicted for the user's organisms. This approach is similar to that used by ", url_FAPROTAX, "."),
           ...
           "Predict traits with metabolic networks" = div(
             ...
             p(h4("Output format")),
             p("The *csv for fluxes and for network model can be downloaded. The higher the fluxes, the faster the reaction or more product that is formed. The flux of substrate is initially set to -1000. When the tool reports the number of end products predicted, it counts those with flux >1 (<0.1% of initial value of flux of the substrate).")
           ),
           "Predict traits with machine learning" = div(
             p(h3("Predict traits with machine learning")),
             p("This tool predicts traits for an organism from its genome using machine learning.  After the user selects gene functions for a genome, the tool uses a machine learning algorithm (random forest classifier) to predict traits."),
             ...
             p(h4("Output format")),
             p("The *csv for probabilities of predicted traits can be downloaded. The probabilities range from 0 to 1.  When the tool reports the number of traits predicted, it counts those with probability >50%."),
  ...
  output$downloadModel_2 <- create_download_handler("random_forest_methanogenesis", function() load_model_methanogenesis(), file_type = "rds")
    # }
  # },
  # ignoreNULL = TRUE,  ignoreInit = TRUE, label="update_choices_taxa") # Set ignoreInit = TRUE to stop function from running on start up
}
