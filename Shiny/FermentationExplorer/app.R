  ...
  #Draw Enzymatic Reactions in Graph
    graph_enzymes = function(s, reference_model=NULL, name=c("NAD+", "NADH", "ATP", "ADP", "Orthophosphate", "H2O", "H+", "CO2"), show_flux=FALSE, color_vertices=FALSE){
      ...
      if(show_flux==TRUE)
      {
        ...
        #Get graph
        df = df %>% select(reactant, product, officialName, subsystem, flux)
  ...
  tagList(
    fluidPage(
      ...
      #Navigation bar
        #Color
          tags$style(HTML("
            ...
            .navbar-default .navbar-nav > .active > a:hover {color:black;background-color:grey50;}
          ")),
        ...
        #Justification of drop down tabs 
        #6th tab justified at right; all others justified at left
      ...
      #Buttons on home page
        #Action button
        tags$head(
         #Box to hold icons and text
         tags$style('
          .home-button-box {width:100%; height:100%; background-color:white; transition: 0.5s ease; position: relative; object-fit: scale-down;}
        '),
         
         tags$style('
          .home-button-box:hover {-webkit-transform: scale(1.05); -ms-transform: scale(1.05); transform: scale(1.05); } 
        '),

         #Icon
         tags$style('        
          .home-button-icon {border-radius: 5px; width:100%; height:100%; background-color: white; border: 0 ; padding-top: 0px; object-fit: scale-down; }
        '),
        ...
        '),
         ...
         #Grid for aligning icon and text
         tags$style('
        .home-button-grid {
                    display: grid;
        ...
        '),
         ...
         #Action buttons
         ##Need separate left and right buttons to align properly in boxes
         tags$style('
          .home-action-button-right { position: absolute; top:0px; left: 0px; width: 260px; height: 100%; opacity: 0;}'
         ),

         tags$style('
          .home-action-button-left { position: absolute; top:0px; right: 0px; width: 260px; height: 100%; opacity: 0;}'
         ),
        
      #Footer
        tags$style('
          .footer {font-size: 12px; display:inline-block; vertical-align:top; text-align:right;
        ...
        #Query Builder
      tags$style('
          #querybuilder_group_0 {border:0px solid white; background-color: #F5F6F6}
          
          .query-builder {font-size: 12px;}
        '),
      ),
        ...
        #Home
          tabPanel(
            ...
            fluidRow(
              ...
              column(
                div(
                  home_button(button_name = 'jump_predict_from_taxonomy', position="right", icon_background_color="#ef4146", image_name="predict_from_taxonomy", title = "Predict traits from taxonomy", subtitle="Just provide names of taxa")
                ),
                width=6, align= "left"
              )
            ),
            ...
            fluidRow(
              column(
                div(
                  home_button(button_name = 'jump_predict_from_genomes', position="left", icon_background_color="#26b784", image_name="predict_from_genomes", title = "Predict traits from genome", subtitle="Use our library of genomes or BYO")
                ),
                width=6, align= "right"
              ),
              column(
                div(
                  home_button(button_name = 'jump_download_database', position="right", icon_background_color="#bb65a8", image_name="download_database", title = "Download database", subtitle="For use in Excel or other programs")
                ),
                width=6, align= "left"
              )
            ),
          ...
          #Database
             tabPanel(
               ...
               title=div(icon("database"), "Database"),
               fluidRow(
                     ...
                     tabPanel(
                         ...
                         sidebarPanel(width = 3, 
                                      tabsetPanel(id = "subtabs",               
                                          ...
                                          tabPanel("File upload",
                                                   ...
                                                   fileInput_custom("file3", "Upload reference reactions",multiple = TRUE,accept = c("text/csv","text/comma-separated-values,text/plain",".csv"), downloadId="downloadReference", downloadlabel="Download example"),
                         ...
                         mainPanel(width = 9,
                                     div(
                                       ...
                                       , fluidRow(
                                         box(
                                           title = "Detailed results",  dataTableOutput("table_predict_from_genomes") %>% withSpinner(color="#3C8DBC"),  width=12, status = "primary", solidHeader = TRUE, collapsible = TRUE
                                         )
                                       ),
          ...
          tabPanel(
            ...
            title=div(icon("download"), "Download"),
            fluidRow(
              column(width=2),
              column(
  ...
  observeEvent(input$tabs, {
    ...
    if(input$tabs == "search_database"){
      ...
      filter_data = reactive({
        ...
        df = filterTable(input$querybuilder_out, df, 'table')
    ...
    if(input$tabs == "predict_from_taxonomy"){
      ...
      get_data = eventReactive({input$predict_from_taxonomy_filter},
        {
          ...
          #validate(need(!is.null(input$predict_from_taxonomy_filter[1]), "Please choose Bergey or NCBI taxonomy"))
      ...
      predict_traits <- reactive({
        ...
        for(i in 1:nrow(query_taxonomy))
        {
          ...
          matching_traits_all = matching_organisms %>% select(Fermentative_ability, Substrates_for_end_products, Major_end_products, Minor_end_products)
          ...
          }else
          {
            ...
            matching_traits_all = matching_traits_all %>% select(-c(Major_end_products, Minor_end_products))
            ...
            join_1 = df %>% select(rn:colnames(df[ncol(df)]))
            matching_traits_all = matching_traits_all %>% full_join(y = join_1, by = "rn")
            matching_traits_all = matching_traits_all %>% select(-End_products, -rn)
